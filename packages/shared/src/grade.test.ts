...
import { esquemaPaginaGrade, esquemaInsercao, esquemaAtualizacao, esquemaRemocao } from './grade';
...
test('paginaGrade aplica defaults e coage números', () => {
  ...
  const r2 = esquemaPaginaGrade.safeParse({
    esquema: 'dbo',
    tabela: 'C',
    pagina: '2',
    tamanho: '50',
...
test('insercao aceita valores e rejeita sem tabela', () => {
  expect(
    esquemaInsercao.safeParse({
      esquema: 'dbo',
      tabela: 'C',
      valores: { nome: 'Ana', idade: 5, ativo: true, obs: null },
    }).success,
  ).toBe(true);
...
test('atualizacao e remocao exigem chave', () => {
  ...
  expect(esquemaRemocao.safeParse({ esquema: 'dbo', tabela: 'C', chave: { id: 1 } }).success).toBe(
    true,
  );
